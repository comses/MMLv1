...
#        rate = '%srate%04d' % (p, o)    # This variable might be used if bedrock weathering is ever implemented
    ...
    #NOTE: this following little section that calculates flow contribution from C-factor is specific to use with the MMLv1. The "normal version" of r.landscape.evol takes this as an input variable, and future version of the MMl will pass it that way too! Right now, we do a linear scaling from C to flow contribution so that forests only allow 15% of the water to drain downslope, but bareland lets 95% of the water drain off.
    flowcontrib = '%sTEMPORARY_flowcontrib%04d' % (pid, o)
    grass.mapcalc("${flowcontrib} = (0.001187 * ${C}) + -0.012812", quiet = "True", C = C, flowcontrib = flowcontrib)
